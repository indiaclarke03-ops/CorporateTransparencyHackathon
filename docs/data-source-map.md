...
| Need | Tool / dataset | Fields | Status |
|---|---|---|---|
| Owners and ultimate owners | Sayari `GET /v1/ubo/{id}` (upward); `GET /v1/downstream/{id}` (owned entities). Parameters: `min_depth`, `max_depth` (default 4), `limit` (default 10, max 50), `psa`, `min_shares`, `include_unknown_shares`, `exclude_former_relationships`, `relationships` | Response `data[]` of `source`, `target`, `path`. Relationship types include `has_shareholder`, `shareholder_of`, `has_beneficial_owner`, `beneficial_owner_of`, `has_owner`, `owner_of`. Percentages: each hop is `{entity, field, relationships}`, and the share is `relationships[<field>].values[].attributes.shares[].percentage` (`TraversalRelationshipData`, `RelationshipInfo`, `SharesProperties` in the spec); officer titles are `attributes.position[].value` | **Confirmed** (`sayari/openapi.yml`: `traversal_ubo`, `traversal_ownership`, `Relationships` enum, `AttributeDetails.shares`); `has_shareholder` seen in **Pilot** |
...
| B36 | The frontend's `public/fixtures/serniya.json` (PR #2) still uses `edge_counts`. Align it with the backend's `relationship_count` together with whoever owns the frontend | Resolved 25 Sep 2026: `lib/types.ts` and the node panel use `relationship_count`; `scripts/build_showcase.py` copies the audited root fixtures to `public/fixtures/` |
...
| B47 | `tests/test_scoring.py` imports `pytest`, which isn't in `requirements.txt`, so it only runs through a local shim. Add a `requirements-dev.txt` with pytest and a single test command (or CI) | Later |
| B48 | The Claude Code Tradeverifyd MCP connector returned HTTP 401 on 25 Sep 2026 (the repo adapter's token is separate). Refresh the token before the live pilot. The dossiers now say trade lanes were not retrieved in this pass, not that there was an authorization error | Open (needed for 2f step 2) |
| B49 | The `/traceability` showcase reads `lib/generated/showcase.json`, which `scripts/build_showcase.py` writes. Re-run the script after changing research, fixtures or the backlog (or add it to the build) so the page doesn't go stale | Later |
| B50 | Typology cases (`scripts/build_cases.py`) cite S43–S48 (Treasury SM1058/JY2164, CBP Hoshine WRO, DHS UFLPA list, DOJ M.D. Fla. Amarvel indictment, DOJ Feeding Our Future sentencing), found with Tavily on 25 Sep 2026. Add them to `research/sources.json` with content checks (S47 is title-only), and re-run Tradeverifyd for the new cases once the token is refreshed (B48) | Open |
